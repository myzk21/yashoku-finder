...
// 画像プレビュー
    ...
    Sortable.create(steps, {
        animation: 150,
        handle: '.handle',
        onEnd: function(evt) {//手順の入れ替えが終わったときに起動
    ...
    //  var stepDelete = document.getElementById('step-delete');
    steps.addEventListener('click', function(evt) {
        if (evt.target.classList.contains('step-delete') || evt.target.closest('.step-delete')) {
            ...
            var items = steps.querySelectorAll('.step');//もう一度取得する
            items.forEach(function(item, index) {
                item.querySelector('.step-number').innerHTML = '手順' + (index + 1);
            });
        }
    ...
    addStep.addEventListener('click', function(evt) {
