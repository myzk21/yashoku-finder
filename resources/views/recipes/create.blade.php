<x-app-layout>
    ...
    <form action="{{ route('recipe.store') }}" method="POST" class="w-[80%] p-4 mx-auto bg-white rounded" enctype="multipart/form-data">
        @csrf
        {{ Breadcrumbs::render('create') }}
        <div class="grid grid-cols-2 rounded my-4">
          <div class="col-span-1">
            <img id="preview" class="object-cover w-full aspect-video" src="/images/noImage.jpg" alt="recipe-image">{{--imageの識別子jpg注意--}}
            <input type="file" id="image" name="image" class="border border-gray-300 p-2 mb-4 w-full rounded">
          </div>
          <div class="col-span-1 p-4">
            <div class="flex">
                <label for="title" class="block text-xl mb-4">レシピ名</label><span class="text-red-500 ml-2">必須</span>
            </div>
            <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="レシピ名" class="border border-gray-300 p-2 mb-4 w-full rounded">
            <div class="flex">
                <label for="description" class="block text-xl mb-4">説明</label><span class="text-red-500 ml-2">必須</span>
            </div>
            <textarea name="description" for="description" placeholder="レシピの説明" class="border border-gray-300 p-2 mb-4 w-full rounded">{{ old('description') }}</textarea>

            {{--材料--}}
            <div class="flex">
                <h4 class="text-xl mb-4">材料</h4><span class="text-red-500 ml-2">必須</span>
            </div>
            <div id="ingredients">
                @php
                ...
                @for($i = 0; $i < 3; $i++)
                    <div class="ingredient flex items-center mb-4">
                        @include('components.bars-3'){{--アイコン--}}
        ...
        <hr class="my-4">{{--横線--}}
        <div class="flex">
            <h4 class="text-xl mb-4">作り方</h4><span class="text-red-500 ml-2">必須</span>
        </div>
        <div id="steps">
            @php
            ...
            @if(is_null($old_steps))
                ...
                <div class="step flex justify-between items-center mb-2">
                    @include('components.bars-3'){{--ハンバーガーアイコン--}}
                    <p class="step-number w-16">手順{{$i}}</p>
                    <input type="text" name="steps[]" placeholder="手順を入力" class="border border-gray-300 p-2 w-full rounded">{{--入力欄--}}
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-10 h-10 ml-4 step-delete text-gray-600">{{--ゴミ箱アイコン--}}
                    <path fill-rule="evenodd" d="M16.5 4.478v.227a48.816 48.816 0 013.878.512.75.75 0 11-.256 1.478l-.209-.035-1.005 13.07a3 3 0 01-2.991 2.77H8.084a3 3 0 01-2.991-2.77L4.087 6.66l-.209.035a.75.75 0 01-.256-1.478A48.567 48.567 0 017.5 4.705v-.227c0-1.564 1.213-2.9 2.816-2.951a52.662 52.662 0 013.369 0c1.603.051 2.815 1.387 2.815 2.951zm-6.136-1.452a51.196 51.196 0 013.273 0C14.39 3.05 15 3.684 15 4.478v.113a49.488 49.488 0 00-6 0v-.113c0-.794.609-1.428 1.364-1.452zm-.355 5.945a.75.75 0 10-1.5.058l.347 9a.75.75 0 101.499-.058l-.346-9zm5.48.058a.75.75 0 10-1.498-.058l-.347 9a.75.75 0 001.5.058l.345-9z" clip-rule="evenodd" />
                    </svg>
                </div>
                @endfor
        ...
        <hr class="my-6">{{--横線--}}
        <div class="flex">
            <h4 class="text-xl mb-4">カテゴリー</h4><span class="text-red-500 ml-2">必須</span>
        </div>
        ...
        <hr class="my-6 border-gray-500">{{--横線--}}
