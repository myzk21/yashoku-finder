<x-app-layout>
    ...
    <form action="{{ route('recipe.update', ['id' => $recipe->id]) }}" method="POST" class="w-[80%] p-4 mx-auto bg-white rounded" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        {{ Breadcrumbs::render('edit') }}
        <div class="grid grid-cols-2 rounded my-4">
          <div class="col-span-1">
            <img id="preview" class="object-cover w-full aspect-video" src="{{ $recipe->image }}" alt="recipe-image">{{--imageの識別子jpg注意--}}
          ...
          <div class="col-span-1 p-4">
            ...
            <input type="text" id="title" name="title" value="{{ $recipe->title }}" placeholder="レシピ名" class="border border-gray-300 p-2 mb-4 w-full rounded">
            <label for="description" class="block text-xl mb-4">説明</label>
            <textarea name="description" for="description" placeholder="レシピの説明" class="border border-gray-300 p-2 mb-4 w-full rounded">{{ $recipe->description }}</textarea>
          ...
          <div class="text-center">
                @foreach($categories as $c)
                    <label for="category{{ $c->id }}" class="mr-3">
                        {{-- <input type="checkbox" name="categories[]" id="category{{ $c->id }}" value="{{ $c->id }}" {{ (is_array($recipe->categories)) && in_array($c->id, $recipe->categories) ? 'checked' : '' }}> --}}
                        <input type="checkbox" name="categories[]" id="category{{ $c->id }}" value="{{ $c->id }}" {{ $recipe->categories->contains('id', $c->id) ? 'checked' : '' }}>
                        {{ $c->name }}
                    </label>
                @endforeach
          </div>
